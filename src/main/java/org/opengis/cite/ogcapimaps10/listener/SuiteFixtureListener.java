...
public class SuiteFixtureListener implements ISuiteListener {
	...
	void processSuiteParameters(ISuite suite) {
		...
		parseDatetimeSubsetCrsInteractiveResults(suite);
	}

	/**
	 * Parses the interactive test parameters for A.29 Req 29/E (CRS interpretation) and
	 * stores the result as a suite attribute.
	 * @param suite the TestNG suite instance
	 */
	private void parseDatetimeSubsetCrsInteractiveResults(ISuite suite) {
		Map<String, String> params = suite.getXmlSuite().getParameters();
		String enabledStr = params.get(TestRunArg.DATETIME_SUBSET_CRS_INTERACTIVE_ENABLED.toString());
		boolean enabled = "true".equalsIgnoreCase(enabledStr);
		boolean crsInterpretationCorrect = false;
		if (enabled) {
			String correctStr = params.get(TestRunArg.DATETIME_SUBSET_CRS_RESULT_CORRECT.toString());
			crsInterpretationCorrect = "true".equalsIgnoreCase(correctStr);
		}
		DatetimeSubsetCrsInteractiveTestResult result = new DatetimeSubsetCrsInteractiveTestResult(enabled,
				crsInterpretationCorrect);
		suite.setAttribute(SuiteAttribute.DATETIME_SUBSET_CRS_INTERACTIVE_TEST_RESULT.getName(), result);
	}
