...
/**
 * Utility methods for interactive CTL tests related to JPEG map content verification.
 * These methods are called from CTL scripts (interactive-jpeg.xml) to build map request
 * URLs for manual verification of Part B (color representation) and Part C (portrayal
 * consistency).
 */
public final class JpegInteractiveTestUtils {
	...
	private JpegInteractiveTestUtils() {
		// Utility class, no instantiation
	}

	/**
	 * Builds a map request URL for JPEG content verification (Part B). Returns the map
	 * URL that can be used in an HTML img tag to display the map as JPEG.
	...
	public static String buildMapRequestWithBbox(String landingPageUrl, String bbox) {
		String mapUrl = findMapUrl(landingPageUrl);
		if (mapUrl == null) {
			return NOT_FOUND_PREFIX + landingPageUrl + "/map?f=jpeg&bbox=" + bbox;
		}
		String separator = mapUrl.contains("?") ? "&" : "?";
		return mapUrl + separator + "f=jpeg&bbox=" + bbox;
	}
