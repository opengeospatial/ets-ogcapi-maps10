 ...
 * Assertions verified:
 * <ul>
 * <li>[Req28/A-broad] {@code datetime=1900/2099} broad interval → HTTP 200, non-empty
...
public class DatetimeResponse extends CommonFixture {
	 ...
	 */
	@Test(description = "A.28 Req 28/A (broad): datetime=1900/2099 broad interval → HTTP 200, non-empty body.")
	...
	/**
	 * Validates that the {@code OGCAPI-datetime} header value is a structurally valid RFC
	 * 3339 instant or interval (using Z notation). Returns an error string, or
	...
	/** Finds the dataset-level map URL from the landing page, or falls back to /map. */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Iterates {@code /collections} to find the first collection with no temporal extent
	 * and a map link. Returns the map URL, or {@code null} if none is found.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Iterates {@code /collections} to find the first collection with a temporal extent
	 * and a map link. Returns the map URL, or {@code null} if none is found.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Returns {@code true} if the collection's extent has a {@code temporal} property,
	 * indicating its resources are associated with a temporal geometry.
	 */
	...
	/**
	 * Returns {@code true} if the collection's extent has no {@code temporal} property,
	 * indicating its resources are not associated with a temporal geometry.
	 */
